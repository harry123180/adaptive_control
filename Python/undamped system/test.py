...
def cuboid_data(center, size):
    ...
    return x, y, z
def coordinate(ax,origin):
    #origin = [x,y,z,a,r,b]
    #origin = [0,1,2,3,4,5]
    # Here we create the arrows:
    arrow_prop_dict = dict(mutation_scale=20, arrowstyle='->', shrinkA=0, shrinkB=0)
    a = Arrow3D([origin[0], (1+origin[0])*cos(origin[3])], [origin[1], (1+origin[1])*cos(origin[4])], [origin[2], (1+origin[2])*cos(origin[5])], **arrow_prop_dict, color='r')
    print([origin[0], (1+origin[0])*cos(origin[3])], [origin[1], (1+origin[1])*cos(origin[4])], [origin[2], (1+origin[2])*cos(origin[5])])
    ax.add_artist(a)
    a = Arrow3D([0, 0], [0, 1], [0, 0], **arrow_prop_dict, color='b')
    ax.add_artist(a)
    a = Arrow3D([0, 0], [0, 0], [0, 1], **arrow_prop_dict, color='g')
    ax.add_artist(a)
    a = Arrow3D([0, 1], [0, 1], [0, 1], **arrow_prop_dict, color='black')
    ax.add_artist(a)
